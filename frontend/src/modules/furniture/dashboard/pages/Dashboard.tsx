...
import MaintenanceModal from '@/modules/furniture/maintenance/components/MaintenanceModal';
...
const Dashboard = () => {
  ...
  const grandTotalMuebles = departmentSummary.reduce((acc, curr) => acc + curr.muebles, 0);
  ...
  const handleSaveMaintenance = async (_id: number, data: any) => {
    await api.post(`/furniture/maintenances/`, data);
            ...
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-white/95 backdrop-blur-md z-10 shadow-sm">
                <tr>
                  ...
                  <th className="px-6 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100"># Muebles</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {departmentSummary.map((dep, idx) => (
                  <tr key={idx} className="hover:bg-emerald-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500 text-center">{dep.muebles}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-5 border-t border-gray-100 bg-gray-50/80 shrink-0 flex justify-between items-center">
             ...
             <div className="flex items-center gap-6">
                <div className="text-right">
                  <p className="text-xs text-gray-400 font-medium uppercase mb-0.5">Muebles</p>
                  <span className="font-medium text-gray-600">{grandTotalMuebles} unid.</span>
                </div>
                <div className="text-right">
        ...
        <div className="bg-white/90 backdrop-blur-2xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white overflow-hidden flex flex-col h-[400px]">
              ...
              <tbody className="divide-y divide-gray-50">
                {categorySummary.map((cat, idx) => (
                  <tr key={idx} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500 text-center">{cat.muebles}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-5 border-t border-gray-100 bg-gray-50/80 shrink-0 flex justify-between items-center">
             ...
             <div className="flex items-center gap-6">
                <div className="text-right">
                  <p className="text-xs text-gray-400 font-medium uppercase mb-0.5">Muebles</p>
                  <span className="font-medium text-gray-600">{categorySummary.reduce((acc, curr) => acc + curr.muebles, 0)} unid.</span>
                </div>
                <div className="text-right">
