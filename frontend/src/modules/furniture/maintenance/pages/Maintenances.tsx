...
import { generateProductPDF, generateBulkProductsPDF } from '@/modules/furniture/inventory/utils/productPdfGenerator';
...
const Maintenances = () => {
  ...
  const fetchLogs = async () => {
    try {
      const res = await api.get('/furniture/maintenances/');
      setLogs(res.data);
    } catch (error) {
  ...
  const handleViewProduct = async (productId: number) => {
    try {
      const res = await api.get(`/furniture/products/${productId}/`);
      setSelectedProduct(res.data);
      setIsViewModalOpen(true);
    } catch (error) {
      toast.error('Error al cargar detalles del mueble');
    }
  };
  ...
  const handlePrintProduct = async (productId: number) => {
    try {
      ...
      toast.error('Error al cargar datos del mueble para imprimir');
    }
  };
  ...
  const handleExport = () => {
    const headers = ['Fecha', 'Código Mueble', 'Nombre Mueble', 'Realizado Por', 'Costo', 'Estado Resultante', 'Descripción'];
  ...
  const handleExportTabla = () => {
    ...
    try {
      ...
      applyAutoTable(doc, {
        startY: 35,
        head: [['Fecha', 'Codigo Mueble', 'Mueble', 'Tecnico', 'Costo', 'Estado', 'Descripcion']],
        body: tableData,
        theme: 'grid',
  ...
  const handleExportFichas = async () => {
    ...
    try {
      const [mantRes, prodRes] = await Promise.all([
        api.get('/furniture/maintenances/'),
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-8">
        <div className="p-4 border-b border-gray-100 bg-gray-50/50">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Buscar por mueble, técnico o descripción..."
              value={searchTerm}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">Mueble</th>
