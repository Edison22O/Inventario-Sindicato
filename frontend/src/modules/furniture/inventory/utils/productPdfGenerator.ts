...
const addProductPageToDoc = async (
  ...
  doc.text('Ficha Técnica de Mueble', pageWidth / 2, currentY, { align: 'center' });
  currentY += 8;
  ...
  const specsData = [
    ['Marca:', product.marca || '-', 'Material:', product.material || '-'],
    ['Dimensiones:', product.dimensiones || '-', 'Color:', product.color || '-'],
...
export const generateProductPDF = async (product: Product) => {
  ...
  try {
    const mantRes = await api.get(`/furniture/maintenances/?product=${product.public_id}`);
