...
import type { Product, Department, Category, Supplier } from '@/shared/types';
import ProductModal from '@/modules/furniture/inventory/components/ProductModal';
import ProductViewModal from '@/modules/furniture/inventory/components/ProductViewModal';
...
const DiscardedProducts = () => {
  ...
  const handleSaveProduct = async (formData: FormData) => {
    if (selectedProduct) {
      await api.patch(`/furniture/products/${selectedProduct.id}/`, formData);
    } else {
  ...
  const handleExport = () => {
    const headers = ['Código', 'Nombre', 'Marca', 'Material', 'Dimensiones', 'Color', 'Departamento', 'Categoría', 'Proveedor', 'Estado', 'Cantidad', 'Costo', 'Fecha de Compra'];
  ...
  const handleExportFichas = async () => {
    ...
    try {
      const mantRes = await api.get('/furniture/maintenances/');
      await generateBulkProductsPDF(
        filteredProducts, 
        mantRes.data, 
        `Bajas_Fichas_${new Date().toISOString().split('T')[0]}.pdf`
      );
    } catch (error) {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (const d of depts) {
          if (!deptMap[d]) {
            const res = await api.post('/furniture/departments/', { name: d, description: '' });
        ...
        for (const c of cats) {
          if (!catMap[c]) {
            const res = await api.post('/furniture/categories/', { name: c, description: '' });
            catMap[c] = res.data.id;
          }
        }
        
        toast.loading('Importando muebles. Esto puede tomar un momento...', { id: 'import' });
        ...
        for (const row of jsonData) {
          ...
          const basePayload = {
            ...
            material: row['MODELO'] ? row['MODELO'].toString() : '',
            dimensiones: row['SERIE'] ? row['SERIE'].toString() : '',
          ...
          try {
            if (diffCents !== 0 && cantOriginal > 1) {
              await api.post('/furniture/products/', { ...basePayload, codigo: cod, cantidad: cantOriginal - 1, costo: costoUnitario });
              count++;
              await api.post('/furniture/products/', { ...basePayload, codigo: `${cod}-aj`, cantidad: 1, costo: Math.round((costoUnitario + diffCents) * 100) / 100 });
              count++;
            } else {
              await api.post('/furniture/products/', { ...basePayload, codigo: cod, cantidad: cantOriginal, costo: costoUnitario });
              count++;
            }
          } catch (postErr) {
        ...
        toast.success(`¡${count} muebles importados con éxito!`, { id: 'import' });
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gradient-to-r from-gray-50/80 to-white/50">
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Marca/Material</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Dimensiones</th>
