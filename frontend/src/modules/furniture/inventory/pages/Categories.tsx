...
import CategoryModal from '@/modules/furniture/inventory/components/CategoryModal';
...
const Categories = () => {
  ...
  const handleSaveCategory = async (data: Partial<Category>) => {
    if (selectedCategory) {
      await api.put(`/furniture/categories/${selectedCategory.public_id}/`, data);
    } else {
      await api.post('/furniture/categories/', data);
    }
    fetchCategories();
  };
  ...
  const handleDeleteCategory = async (e: React.MouseEvent, id: number) => {
    ...
    if (await confirmDialog('¿Estás seguro de que deseas eliminar esta categoría?')) {
      try {
        await api.delete(`/furniture/categories/${id}/`);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredCategories.map((cat) => (
              <div key={cat.id} className="relative group">
                <Link 
                  to={`/furniture/categories/${cat.public_id}`}
