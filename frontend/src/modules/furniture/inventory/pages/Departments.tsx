...
const Departments = () => {
  ...
  const fetchDepartments = async () => {
    setLoading(true);
    try {
      const response = await api.get('/furniture/departments/');
  ...
  const handleSaveDepartment = async (data: Partial<Department>) => {
    if (selectedDepartment) {
      await api.put(`/furniture/departments/${selectedDepartment.public_id}/`, data);
    } else {
      await api.post('/furniture/departments/', data);
    }
    fetchDepartments();
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredDepartments.map((dep) => (
              <div key={dep.id} className="relative group">
                <Link 
                  to={`/furniture/departments/${dep.public_id}`}
