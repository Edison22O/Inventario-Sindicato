...
import { FileBarChart, Download, Calendar, Truck, TrendingUp, AlertCircle, Filter, User, Wrench, DollarSign, Layers } from 'lucide-react';
...
const VehicleReports = () => {
  ...
  const uniqueVehicles = Array.from(
    new Set([...trips.map(t => t.vehicle_placa), ...maintenanceRecords.map(m => m.vehicle_placa)])
  ).filter(Boolean).sort();
  ...
  }, [maintenanceRecords, dateRange, selectedVehicle, selectedActividad]);

  // Totales
  const totalCostoCombustible = filteredTrips.reduce((acc, t) => acc + parseFloat(t.costo_combustible_viaje || '0'), 0);
  ...
  const getFilterText = () => {
    ...
    return `Período: ${timeText} | Vehículo: ${selectedVehicle || 'Todos'} | Conductor: ${selectedConductor || 'Todos'} | Actividad: ${selectedActividad || 'Todas'}`;
  };
  ...
  const handleExportPDF = () => {
    ...
    if (reportCategory === 'todos' || reportCategory === 'mantenimiento') {
      ...
      maintTableBody.push([
        { content: 'SUBTOTAL MANTENIMIENTOS', colSpan: 5, styles: { fillColor: [254, 243, 199], textColor: [146, 64, 14], fontStyle: 'bold', halign: 'right' } },
      ...
      applyAutoTable(doc, {
        startY: currentY,
        head: [['Fecha', 'Placa', 'Actividad / Servicio', 'Taller / Proveedor', 'Notas / Detalles', 'Costo Total']],
        body: maintTableBody,
        theme: 'grid',
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto pb-32 relative">
      ...
      <div className="relative z-10 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2 text-base font-medium">Análisis detallado de consumo de combustible y gastos de mantenimiento de la flota.</p>
        </div>
        <button 
          onClick={handleExportPDF}
          disabled={filteredTrips.length === 0 && filteredMaintenances.length === 0}
